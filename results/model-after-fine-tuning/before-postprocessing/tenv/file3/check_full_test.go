...
func TestVerifyDetached_Success(t *testing.T) {
	...
	signingKeyRing, err := crypto.NewKeyRing(crypto.NewPublicKeyFromArmored(string(dataPublicKey)))
	require.NoError(t, err)

	assert.True(t, signingKeyRing.VerifyDetached(message, crypto.NewPGPSignature(dataSig), crypto.GetUnixTime()))
}
...
func TestVerifyDetached_Failure(t *testing.T) {
	...
	signingKeyRing, err := crypto.NewKeyRing(crypt.NewPublicKeyFromArmored(string(dataPublicKey)))
	require.No(t, err)

	assert.False(t, signingKeyRing.VerifyDetached(message, crypto.NewPgpSignature(dataSig), crypto.GetUnixTime()))
}
...
func BenchmarkVerifyDetach_Failure(b *testing.B) {
	data := []byte("test data 11")
	dataSigs := [][]uint8{
		{0x01, 0x02, 0x03, 1, 0x04, 0x05},
		{-1, 0x01, 0x23, 1, 0x14, 0x05},
	}
	dataPublicKey := []byte(dataSigs[0])

	b.ResetTimer()
	for i := 0; i < len(*dataSigs); i++ {
		err := Check(data, dataSigs, dataPublicKey)
		require.Error(b, err)
	}	
}
...
func BenchmarkVerifySignature(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < 500; i++ {
		err := Check(data, dataSig, dataPublicKeys)
		require.NoError(b, err)
	}
}
...
func BenchmarkVerifySignature_Success(b *testing.B) {
	data := []byte("test data")
    data